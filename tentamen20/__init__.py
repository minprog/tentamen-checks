...
"""
    with helpers.replace_main("bundled.c", main):
        with helpers.logged_check_factory("bundled") as run_check:
            (run_check()
                .stdout(r'[\s?3\s?,\s?4\s?,\s?5\s?]', str_output='[3, 4, 5]')
                .stdout(r'[\s?6\s?,\s?7\s?,\s?8\s?]', str_output='[6, 7, 8]')
                .stdout(r'[\s?9\s?]', str_output='[9]')
                .stdout(r'[\s?20\s?,\s?15\s?,\s?14\s?,\s?16\s?]', str_output='[20, 15, 14, 16]')
                .stdout(r'[\s?37\s?,\s?45\s?,\s?5\s?,\s?18\s?]', str_output='[37, 45, 5, 18]')
                .stdout(r'[\s?4\s?,\s?3\s?,\s?2\s?]', str_output='[4, 3, 2]')
                .stdout(r'[\s?1\s?,\s?2\s?,\s?3\s?,\s?4\s?]', str_output='[1, 2, 3, 4]')
                .stdout(r'[\s?1\s?]', str_output='[1]')
                .stdout(r'[\s?2\s?]', str_output='[2]')
                .stdout(r'[\s?3\s?]', str_output='[3]')
                .stdout(r'[\s?4\s?]', str_output='[4]')
            )
